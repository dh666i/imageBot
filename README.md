...
页面底部可以直接选择方形、横图或竖图。其他图片参数在 `更多设置` 中。
...
打开右上角 `设置` 后，在 `本地文件` 中可以查看图片、历史记录和日志占用空间，也可以分别清理。
...
更新时不会覆盖你的 `config.ini`，也不会删除 `outputs/` 里的图片和 `logs/` 日志。

更新完成后，关闭黑色启动窗口，再重新双击 bat 启动即可完全生效。
